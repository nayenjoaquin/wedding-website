...
export const TimeLineItem = (props: TimeLineItemProps) => {
    ...
    return(
        ...
        <p className={`font-handwritten bg-white p-1 absolute text-xl md:text-3xl ${(index%4)<2 ? 'top-5' : 'bottom-5'} ${(index%2)==0 ? 'right-[-50px]':'left-[-50px]'} w-full flex justify-center items-center`}>{t('text'+(index+1))}</p>
        </motion.div>
    )
}